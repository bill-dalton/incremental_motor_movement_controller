...
//motion constants
static const float BR_JOINT_DEGREES_PER_ENCODER_COUNT = 0.03000; //June 2019
static const float SL_JOINT_DEGREES_PER_ENCODER_COUNT = 0.01408; //0.01284=360/1440/17.754=360/SL_JOINT_ENCODER_PPR/SL_JOINT_ENCODER_GEAR_RATIO
static const float UR_JOINT_DEGREES_PER_ENCODER_COUNT = 0.01808; //0.01284=360/1440/13.825
static const float EL_JOINT_DEGREES_PER_ENCODER_COUNT = 0.005698; //0.01284=360/1440/43.8749
static const float LR_JOINT_DEGREES_PER_ENCODER_COUNT = 0.057143; //0.057143=360/(600*4)/2.625 June 2019 configuration
//static const float WL_JOINT_DEGREES_PER_ENCODER_COUNT = 0.010141226; //xl430, not stepper
//static const float WR_JOINT_DEGREES_PER_ENCODER_COUNT = 0.010986328; //xl430, not stepper
...
void commandedIncrementalMotorMovementCallback(const std_msgs::String& the_command_msg_) {
  ...
  */
  ...
  //check for Command 400 to set joint position  values to zero
  if (commanded_incremental_movement > 200.0 && commanded_incremental_movement < 600.0) {
    ...
    //log
    nh.loginfo("cmd 400: reset joint position vars to zero");

    //exit without beginning any movement
    return;
  }// end of command 400

  //check for Command 800 to set stepper postion values to zero
  if (commanded_incremental_movement > 600.0 && commanded_incremental_movement < 1000.0) {
    //Command 800 - set stepper postion values to zero
    float new_stepper_pos_;
    new_stepper_pos_ = commanded_incremental_movement - 800.0;
    ...
    //log
    nh.loginfo("cmd 800: reset stepper postion vars to zero");
...
void readSensors() {
  ...
  if (encoder_direction_reversed == true){
    minion_state.joint_position = -1.0 * ((float) encoder_1_pos) * joint_degrees_per_encoder_count;//TO DO change this to true joint encoder after joint encoder actually installed
  }
  else {
    minion_state.joint_position = ((float) encoder_1_pos) * joint_degrees_per_encoder_count;//TO DO change this to true joint encoder after joint encoder actually installed    
  }
